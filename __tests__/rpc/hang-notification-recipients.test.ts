...
describe("get_hang_notification_recipients", () => {
  ...
  beforeAll(async () => {
    ...
    await adminClient.from("blocks").insert({ blocker_id: host.id, blocked_id: blockedFriend.id });
  });

  afterAll(async () => {
    await cleanupTestData([host.id, friend.id, mutual.id, blockedFriend.id, stranger.id]);
  });

  it("returns friends + mutuals, excludes blocked and strangers", async () => {
    const { data, error } = await adminClient.rpc("get_hang_notification_recipients", {
      p_host_id: host.id,
    });
