...
export const useCreateTravelPlan = () => {
  ...
  return useMutation({
    mutationFn: async (input: CreateTravelPlanInput): Promise<CreateTravelPlanOutput> => {
      const { destination, start_date, duration_days, post_visibility, text } = input;
      ...
      const { data, error } = await supabase.rpc("create_travel_plan_with_post", {
      ...
      if (!data || data.length === 0) throw new Error("Failed to create travel plan");
...
export const useUpdateTravelPlan = () => {
  ...
  return useMutation({
    mutationFn: async (input: UpdateTravelPlanInput): Promise<CreateTravelPlanOutput> => {
      const { travel_plan_id, destination, start_date, duration_days, post_visibility, text } = input;
      ...
      const { data, error } = await supabase.rpc("update_travel_plan_with_post", {
      ...
      if (error) throw error;
      if (!data || data.length === 0) throw new Error("Failed to update travel plan");
...
export const useGetActiveTravelPlan = (userId?: string): UseQueryResult<GetActiveTravelPlanOutput> => {
  const { supabase } = useSupabase();
  const profile = useRequireProfile();
...
export const useGetTravelPlans = (userId?: string): UseQueryResult<{ data: TravelPlan[] }> => {
  const { supabase } = useSupabase();
  const profile = useRequireProfile();
  const targetUserId = userId ?? profile.id;
...
export const useGetTravelPlanLocations = (): UseQueryResult<GetTravelPlanLocationsOutput> => {
  const { supabase } = useSupabase();
  const profile = useRequireProfile();
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.rpc("get_active_travel_plan_locations", {
        p_user_id: profile.id,
      });
      if (error) throw error;
...
export const useGetTravelPlanByPostId = (postId?: string): UseQueryResult<TravelPlanWithCoordinates | null> => {
  const { supabase } = useSupabase();
