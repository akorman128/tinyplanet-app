...
export const useCreateList = () => {
  ...
  return useMutation({
    mutationFn: async (input: CreateListInput): Promise<CreateListOutput> => {
      ...
      const { data: placesData } = await supabase.rpc(
...
export const useUpdatePlace = () => {
  ...
  return useMutation({
    mutationFn: async (input: UpdatePlaceInput): Promise<ListPlace> => {
      ...
      const updatedPlace = (placesData as ListPlace[])?.find(
        (p) => p.id === place_id
      );
      if (!updatedPlace)
...
export const useGetViewableLists =
  (): UseQueryResult<GetViewableListsOutput> => {
    const { supabase } = useSupabase();
    const profile = useRequireProfile();
    ...
    return useQuery({
      ...
      queryFn: async () => {
        ...
        const { data: rawOwnLists, error: ownError } = await supabase
          .from("lists")
          .select("*")
          .eq("user_id", profile.id)
        ...
        if (friendError)
          throw new Error(
            `Failed to fetch friends' lists: ${friendError.message}`
          );
