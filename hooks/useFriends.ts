  ...
  return useMutation({
    mutationFn: async (
      ...
      if (insertError.code === "23505") {
        ...
        if (updateError)
          throw new Error(
            "A friendship record already exists but cannot be updated."
          );
        if (!updateData)
          throw new Error(
            "A friendship request already exists between you and this user."
          );
...
export const useGetFriends = (
  userId?: string
): UseQueryResult<GetFriendsOutput> => {
  const { supabase } = useSupabase();
  const profile = useRequireProfile();
...
export const useGetFriendsOfFriends =
  (): UseQueryResult<GetFriendsOfFriendsOutput> => {
    const { supabase } = useSupabase();
    const profile = useRequireProfile();
...
export const useGetFriendLocations =
  (): UseQueryResult<GeoJSONFeatureCollection> => {
    const { supabase } = useSupabase();
    const profile = useRequireProfile();
      ...
      queryFn: async () => {
        ...
        const features: GeoJSONFeature[] = allLocations.map((loc) => ({
          ...
          properties: {
            ...
            connecting_friend_id: loc.connecting_friend_id,
          },
        }));
...
export const useGetFriendHometownLocations =
  (): UseQueryResult<GeoJSONFeatureCollection> => {
    const { supabase } = useSupabase();
    const profile = useRequireProfile();
      ...
      queryFn: async () => {
        ...
        const features: GeoJSONFeature[] = data.map((loc) => ({
          ...
          properties: {
            ...
            avatar_url: loc.avatar_url ?? undefined,
...
export const useSearchFriends = (
  query?: string
): UseQueryResult<SearchFriendsOutput> => {
  const { supabase } = useSupabase();
  const profile = useRequireProfile();
...
export const useGetFriendshipStatus = (
  targetUserId?: string
): UseQueryResult<{ status: FriendshipDisplayStatus }> => {
  const { supabase } = useSupabase();
  const profile = useRequireProfile();
...
export const useGetPendingRequests =
  (): UseQueryResult<GetPendingRequestsOutput> => {
    const { supabase } = useSupabase();
    const profile = useRequireProfile();
    ...
    return useQuery({
      ...
      queryFn: async () => {
        ...
        const { data, error } = await supabase
          .from("friendships")
          .select(
            `id, user_a, user_b, status, requested_by, created_at,
          a:profiles!friendships_user_a_fkey (id, full_name, avatar_url, website, hometown, birthday, location),
          b:profiles!friendships_user_b_fkey (id, full_name, avatar_url, website, hometown, birthday, location)`
          )
...
export const useGetMutualsBetweenUsers = (
  targetUserId?: string
...
export const useGetPlatformStatistics =
  (): UseQueryResult<PlatformStatisticsOutput> => {
    const { supabase } = useSupabase();
    const profile = useRequireProfile();
